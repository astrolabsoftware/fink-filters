...
def make_visibility_masks(constraints,observatory,targets,time_range:list):
    ...
    mask_visibility = is_observable(constraints, observatory, targets,
                                    time_range=time_range)
    
    return mask_visibility

def plot_common_sky(nside:int,scheme:str,target_table:Table,
                    mask_common_sky:np.array,title_sky:str):
...
if __name__ == '__main__':
    ...
    for i in range(len(observatories)):
        try:
            ...
            observatory =  Observer(longitude=observatories.longitude[i]*u.deg,
        ...
        time_ranges = simu_night_time_interval(observatory,ref_date[0],n_days,day_bin)
        tstarts.append(time_ranges[0][0].isot)
        tends.append(time_ranges[0][1].isot)
        for time_range in time_ranges:
            mask_visibility_vro = make_visibility_masks(constraints,
                                                              vro,
                                                              targets,
                                                              time_ranges_vro)
            mask_visibility_observatory = make_visibility_masks(constraints,
                                                              observatory,
                                                              targets,
                                                              time_range)
            mask_both_observability = mask_visibility_vro&mask_visibility_observatory
            sky_frac_visibility = mask_both_observability.sum()/\
                                    mask_visibility_vro.sum()
            ...
            title_combi = 'VRO-'+observatories.obs_name[i]+' overlapping skies = '+\
                str("%.2f" %(sky_frac_visibility*100))+' %'
            print(title_combi)
            
            plot_common_sky(nside,scheme,target_table,
                            mask_both_observability,title_combi)
