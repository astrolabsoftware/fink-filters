...
def coord_to_airmass(ra: np.ndarray, 
                     dec: np.ndarray, 
                     midpointTai: np.ndarray) -> np.ndarray:
    """
    ...
    coord = SkyCoord(ra=ra, dec=dec, unit='deg', obstime=time,
                      location=EarthLocation.of_site('Cerro Pachon'))
    airmass = coord.transform_to('altaz').secz.value

    return airmass
 
def mdwarf_flare(diaSource: pd.DataFrame,
                 ...
                 gaiadr3_e_Plx: pd.Series) -> pd.Series:
    ...
    f_mdwarf_flare = f_star & f_band & f_color & f_airmass & f_excursion & f_time_separation & f_quality
