...
def make_mangrove_pdf(path_in, path_out='mangrove_filtered.csv', range_interferometers=230):
    """
    ...
    The index of the galaxy and the stellar mass are not needed in the early_kn_candidates 
    function so they may be removed. They are here to be used for information purposes only.
    ...
    range_interferometers: float
        range of the interferometers in Mpc. Only the galaxies in this range will be considered?
    ...
    pdf_mangrove = pdf_mangrove.loc[:,[
        'idx', # Integer between 0 and N where N is the total number galaxies in initial GLADE catalog used
        ...
        'stellarmass' # Determined stellar mass of the the associated AllWISE object, empty if no association 
    ]]
    ...
    pdf_mangrove.rename(columns={'idx':'galaxy_idx','RA':'ra','dist':'lum_dist'}, inplace=True)
