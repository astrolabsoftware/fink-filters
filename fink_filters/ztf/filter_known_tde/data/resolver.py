...
for name in tns_names:
    r = requests.post(
        "{}/api/v1/resolver".format(APIURL), json={"resolver": "simbad", "name": name}
    )
    if r.json() == []:
        r = requests.post(
            "{}/api/v1/resolver".format(APIURL), json={"resolver": "tns", "name": name}
        )
...
print(
    "Number of TDE found: {}/{}".format(len(pdf_final), len(pdf_final) + len(unknown))
)
