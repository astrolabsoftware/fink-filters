...
def is_rising(
    jd, flux, fluxerr, fid,
    upper=None,
    nsigmas_rise=2, nsigmas_decay=1, nsigmas_slope=3,
    use_slope=True,
    verbose=False
):
    idx0 = flux == flux
...
@pandas_udf(IntegerType(), PandasUDFType.SCALAR)
def is_rising_u(jd: pd.Series, mag: pd.Series, magerr: pd.Series, isdiffpos: pd.Series, fid: pd.Series) -> pd.Series:
    result = []
    ...
    for jd1,mag1,magerr1,isdiffpos1,fid1 in zip(jd, mag, magerr, isdiffpos, fid):
        ...
        result.append(is_rising(jd1, flux1, fluxerr1, fid1, nsigmas_rise=2, nsigmas_decay=1))
...
def prefilter_alerts(df):
    ...
    wanted = [
        '', 'X', 'IR', 'Radio', 'MIR', 'NIR', 'HH', 'HI', 'HII', 'HighPM*',
        'LensedImage', 'LensingEv', 'Maser', 'MolCld', 'PartofCloud', 'Radio(sub-mm)',
        'Blue', 'Possible_lensImage', 'Unknown', 'Radio(mm)', 'denseCore',
        'Radio(cm)', 'UV', 'PN', 'PN?', 'EmObj', 'DkNeb', 'Transient', 'Candidate_LensSystem',
        'FIR', 'multiple_object', 'GravLensSystem', 'Bubble', 'Cloud', 'SFregion', 'Inexistent',
        'gamma', 'GravLens', 'HVCld', 'Candidate_Lens', 'ISM', 'Void', 'RfNeb', 'HIshell', 'Outflow',
        'radioBurst', 'Region', 'Globule', 'outflow?', 'ComGlob', 'GinCl', 'Galaxy', 'AGN', 'GiC', 'Sy1',
        'Sy2', 'AGN_Candidate', 'QSO', 'Seyfert_1', 'Seyfert_2', 'LINER', 'EmG', 'RadioG', 'BClG', 'LSB_G',
        'LensedG', 'LensedQ', 'GroupG', 'PartOfG', 'BLLac', 'GinPair', 'Possible_ClG', 'Possible_G',
        'Possible_GrG', 'GinGroup', 'HII_G', 'Blazar', 'ClG', 'QSO_Candidate', 'Seyfert',
        'Blazar_Candidate', 'StarburstG', 'IG', 'SuperClG', 'PartofG', 'Compact_Gr_G', 'PairG',
        'BLLac_Candidate', 'BlueCompG', 'Seyfert2', 'Seyfert1'
    ]
