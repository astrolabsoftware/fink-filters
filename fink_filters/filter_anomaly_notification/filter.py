...
def anomaly_notification_(df, threshold=10):
    ...
    df : Spark DataFrame with column :
        ...
        lc_features: Dict of dicts of floats. Keys of first dict - filters (fid), keys of inner dicts - names of features
    ...
    >>> mask = anomaly_notification_(df.select('objectId', 'candidate.ra', 'candidate.dec', 'candidate.rb', 'anomaly_score', 'timestamp'))
    >>> print(sum(mask))
    """
